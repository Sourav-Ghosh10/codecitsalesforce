<x-app-layout>
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('call-logs.show', $callLog) }}" class="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-white transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
            </svg>
        </a>
        <div>
            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Edit Call Record</h1>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2">
            <form method="POST" action="{{ route('call-logs.update', $callLog) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="phone_number" id="phone_number" value="{{ old('phone_number', $callLog->phone_number) }}" 
                ...
                <div>
                    ...
                    <select name="dialer_platform" id="dialer_platform" 
                        ...
                        @foreach(App\Models\CallLog::getDialerPlatforms() as $value => $label)
                            <option value="{{ $value }}" {{ old('dialer_platform', $callLog->dialer_platform) == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <label class="relative flex items-center justify-center p-4 border-2 border-gray-200 dark:border-slate-600 rounded-xl cursor-pointer hover:border-indigo-500 dark:hover:border-indigo-400 transition-colors has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 dark:has-[:checked]:bg-indigo-900/20">
                            <input type="radio" name="call_direction" value="Incoming" class="sr-only" {{ old('call_direction', $callLog->call_direction) == 'Incoming' ? 'checked' : '' }}>
                        ...
                        <label class="relative flex items-center justify-center p-4 border-2 border-gray-200 dark:border-slate-600 rounded-xl cursor-pointer hover:border-indigo-500 dark:hover:border-indigo-400 transition-colors has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50 dark:has-[:checked]:bg-indigo-900/20">
                            <input type="radio" name="call_direction" value="Outgoing" class="sr-only" {{ old('call_direction', $callLog->call_direction) == 'Outgoing' ? 'checked' : '' }}>
                            <div class="text-center">
                                <svg class="w-8 h-8 mx-auto mb-2 text-gray-600 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                                </svg>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="datetime-local" name="call_start_time" id="call_start_time" value="{{ old('call_start_time', $callLog->call_start_time->format('Y-m-d\TH:i')) }}" 
                            class="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                            required>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="call_end_time" id="call_end_time" value="{{ old('call_end_time', $callLog->call_end_time->format('Y-m-d\TH:i')) }}" 
                            class="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                            required>
                        @error('call_end_time')
                            <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="call_result" id="call_result" 
                        ...
                        @foreach(App\Models\CallLog::getCallResults() as $value => $label)
                            <option value="{{ $value }}" {{ old('call_result', $callLog->call_result) == $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                    @error('call_result')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="4" 
                        class="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                        placeholder="Add any notes about this call...">{{ old('notes', $callLog->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="next_follow_up_date" id="next_follow_up_date" value="{{ old('next_follow_up_date', $callLog->next_follow_up_date?->format('Y-m-d')) }}" 
                        class="w-full px-4 py-3 border border-gray-300 dark:border-slate-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                ...
                <div class="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 border border-amber-200 dark:border-amber-800">
                    <label for="admin_edit_reason" class="block text-sm font-medium text-amber-800 dark:text-amber-300 mb-2">Reason for Changes *</label>
                    <textarea name="admin_edit_reason" id="admin_edit_reason" rows="3" 
                        class="w-full px-4 py-3 border border-amber-300 dark:border-amber-600 rounded-xl bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 focus:border-transparent"
                        placeholder="Please explain why you are making this change..."
                        required>{{ old('admin_edit_reason', $callLog->admin_edit_reason) }}</textarea>
                    @error('admin_edit_reason')
                        <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @else
                        <p class="mt-2 text-xs text-amber-700 dark:text-amber-400">This reason will be recorded for audit purposes.</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="{{ route('call-logs.show', $callLog) }}" class="px-6 py-3 bg-gray-100 dark:bg-slate-700 hover:bg-gray-200 dark:hover:bg-slate-600 text-gray-700 dark:text-gray-300 rounded-xl font-medium transition-colors">
                        Cancel
                    </a>
                </div>
            </form>
        ...
        <div class="lg:col-span-1">
            ...
            <div class="bg-amber-50 dark:bg-amber-900/20 rounded-2xl p-6 mt-6 border border-amber-100 dark:border-amber-800">
                <h3 class="text-sm font-semibold text-amber-800 dark:text-amber-300 mb-2">Important Notice</h3>
                <p class="text-sm text-amber-700 dark:text-amber-400">
                    You are editing this call record as an Admin/Manager. All changes are logged with the reason you provide above. This is for audit purposes and ensures accountability for any modifications to call records.
                </p>
            </div>
        </div>
    </div>
