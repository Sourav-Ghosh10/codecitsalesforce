<x-app-layout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2.5 py-1 bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-xs font-semibold rounded-full">+{{ $stats['callsGrowth'] ?? 0 }}%</span>
            </div>
            <h3 class="text-3xl font-bold text-gray-900 dark:text-white">{{ number_format($stats['callsToday'] ?? 0) }}</h3>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700 hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-4">
                ...
                <span class="px-2.5 py-1 bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 text-xs font-semibold rounded-full">{{ $stats['tasksGrowth'] ?? 0 }}</span>
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400">Monthly call volume</p>
                    </div>
                ...
                <div class="flex items-end justify-between h-48 gap-3">
                    @php
                        $months = $stats['chartMonths'] ?? ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun'];
                        $values = $stats['chartValues'] ?? [0, 0, 0, 0, 0, 0];
                        $maxValue = max($values) ?: 10;
                    @endphp
                    @foreach($months as $index => $month)
                        ...
                        <div class="flex flex-col items-center flex-1">
                            <div class="w-full bg-indigo-100 dark:bg-indigo-900/30 rounded-t-lg relative group cursor-pointer transition-all hover:bg-indigo-200 dark:hover:bg-indigo-900/50" style="height: {{ max($percentage * 1.5, 10) }}px; min-height: 10px;">
                                <div class="absolute bottom-0 left-0 right-0 bg-indigo-600 rounded-t-lg transition-all duration-300 h-0 group-hover:h-full"></div>
                                <div class="absolute -top-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-gray-900 dark:bg-slate-700 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">{{ number_format($values[$index]) }} Calls</div>
                            </div>
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50 dark:bg-slate-700/50">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Next Follow-up</th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 text-xs font-bold">{{ $initials }}</div>
                                        <div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-tight {{ $client->status_color }}">
                                        {{ $client->status }}
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    @if($lastLog && $lastLog->next_follow_up_date)
                                        <span class="text-xs font-bold {{ $lastLog->next_follow_up_date->isToday() ? 'text-rose-500 animate-pulse' : 'text-indigo-600 dark:text-indigo-400' }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('clients.show', $client->id) }}" class="inline-flex p-2 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-8">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700">
                ...
                <div class="space-y-4">
                    ...
                    <a href="{{ route('clients.show', $task->client_id) }}" class="flex items-start gap-3 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-shadow cursor-pointer group border border-transparent hover:border-gray-100 dark:hover:border-slate-700">
                        <div class="mt-0.5">
                            <div class="w-2.5 h-2.5 rounded-full {{ $task->next_follow_up_date->isToday() ? 'bg-rose-500 shadow-sm shadow-rose-500/50' : 'bg-indigo-400' }}"></div>
                        </div>
                        ...
                        <span class="px-2 py-0.5 bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-400 text-[10px] font-bold rounded-full">Urgent</span>
                        @endif
                    </a>
                    @empty
                    <div class="py-10 text-center">
                        <div class="w-12 h-12 bg-gray-100 dark:bg-slate-700/50 rounded-full flex items-center justify-center mx-auto mb-3">
                ...
                <a href="{{ route('tasks.index') }}" class="w-full mt-6 py-2.5 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors border-t border-gray-100 dark:border-slate-700 uppercase tracking-widest text-center block">
                    View All Tasks
                </a>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700">
                ...
                <div class="space-y-4">
                    ...
                    @endphp
                    <div class="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors cursor-pointer group">
                        <div class="w-12 h-12 bg-{{ $color }}-100 dark:bg-{{ $color }}-900/30 rounded-xl flex flex-col items-center justify-center transition-transform group-hover:scale-110">
                            <span class="text-xs font-bold text-{{ $color }}-600 dark:text-{{ $color }}-400">{{ $event->due_at->format('d') }}</span>
                            <span class="text-[10px] font-medium text-{{ $color }}-500 dark:text-{{ $color }}-400 uppercase">{{ $event->due_at->format('M') }}</span>
                        </div>
                        <div class="flex-1 min-w-0">
                ...
                <a href="{{ route('tasks.index') }}" class="w-full mt-4 py-2.5 text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-xl transition-colors text-center block">View All Events</a>
            </div>

            <!-- Quick Actions -->
            <div class="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-2xl p-6 text-white overflow-hidden relative group">
                ...
                <div class="grid grid-cols-2 gap-4 relative z-10">
                    <a href="{{ route('clients.create') }}" class="flex flex-col items-center gap-3 p-4 bg-white/10 hover:bg-white/20 rounded-xl transition-all hover:-translate-y-1 active:scale-95 border border-white/5">
                    ...
                    <a href="{{ route('clients.index') }}" class="flex flex-col items-center gap-3 p-4 bg-white/10 hover:bg-white/20 rounded-xl transition-all hover:-translate-y-1 active:scale-95 border border-white/5">
                        <div class="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
                            </svg>
                    ...
                    <a href="{{ route('tasks.create') }}" class="flex flex-col items-center gap-3 p-4 bg-white/10 hover:bg-white/20 rounded-xl transition-all hover:-translate-y-1 active:scale-95 border border-white/5">
                        <div class="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a href="mailto:" class="flex flex-col items-center gap-3 p-4 bg-white/10 hover:bg-white/20 rounded-xl transition-all hover:-translate-y-1 active:scale-95 border border-white/5">
                        <div class="w-10 h-10 bg-white/10 rounded-lg flex items-center justify-center">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <x-modal name="followup-reminder" :show="true" maxWidth="md">
        <div class="bg-white dark:bg-slate-800 shadow-xl border border-gray-100 dark:border-slate-700">
            <!-- Modal Header -->
            <div class="px-6 py-4 border-b border-gray-100 dark:border-slate-700 flex items-center justify-between bg-gray-50/50 dark:bg-slate-800">
                <div class="flex items-center gap-2">
                    <div class="w-2 h-2 rounded-full bg-indigo-600 animate-pulse"></div>
                    <h3 class="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Scheduled Follow-ups</h3>
                </div>
                <button x-on:click="$dispatch('close-modal', 'followup-reminder')" class="p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="divide-y divide-gray-100 dark:divide-slate-700">
                    ...
                    <div class="px-6 py-4 hover:bg-gray-50/50 dark:hover:bg-slate-700/20 transition-colors">
                        <div class="flex items-center gap-4">
                            ...
                            <div class="w-10 h-10 shrink-0 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-600 flex items-center justify-center text-white text-xs font-bold shadow-sm">
            ...
            <!-- Modal Footer -->
            <div class="px-6 py-4 bg-gray-50 dark:bg-slate-700/50 border-t border-gray-100 dark:border-slate-700 flex items-center justify-end gap-3">
                <button x-on:click="$dispatch('close-modal', 'followup-reminder')" class="px-4 py-2 text-xs font-bold text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors uppercase tracking-wider">
                    Remind Later
                </button>
                <button x-on:click="$dispatch('close-modal', 'followup-reminder')" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-600/20 active:scale-95">
                    Got it
                </button>
            </div>
        </div>
