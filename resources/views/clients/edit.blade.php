<x-app-layout>
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Update client information - {{ $client->customer_number }}</p>
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('clients.update', $client) }}" class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-slate-700">
                    ...
                    <div class="mb-6">
                        ...
                        <select name="agent_id" id="agent_id"
                            ...
                            @foreach($agents as $agent)
                                <option value="{{ $agent->id }}" {{ old('agent_id', $client->agent_id) == $agent->id ? 'selected' : '' }}>{{ $agent->name }} ({{ $agent->getRoleLabelAttribute() }})</option>
                            @endforeach
                        </select>
                    </div>
                    @else
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="tags" id="tags" value="{{ old('tags', $client->tags) }}" placeholder="e.g: VIP, Enterprise, Retail"
                        class="w-full px-4 py-2.5 bg-gray-50 dark:bg-slate-700 border-0 rounded-xl text-sm text-gray-700 dark:text-gray-300 focus:ring-2 focus:ring-indigo-500">
